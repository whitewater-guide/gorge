...
func (s *scriptNztrc) parseList(gauges chan<- *core.Gauge, measurements chan<- *core.Measurement, errs chan<- error) {
	...
	for code, st := range stations {
		...
		if st.level != nil {
			...
			common = (*st.level)
			level.Scan((*st.level).Measure) //nolint:errcheck // parse failure means null, which is valid
		}
		if st.flow != nil {
			flowUnit = st.flow.Unit
			common = (*st.flow)
			flow.Scan((*st.flow).Measure) //nolint:errcheck // parse failure means null, which is valid
		}
		if flowUnit == "m3/sec" {
			flowUnit = "m3/s"
		}
